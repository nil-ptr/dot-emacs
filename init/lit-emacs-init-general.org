...
*** Primitives
    ...
    - =my-buffer-list-with-base-buffer-of=
      ...
      #+BEGIN_SRC emacs-lisp +n -r -l ";(%s)"
        (defun my-buffer-list-with-base-buffer-of (&optional buffer-or-name)
                ...
                (seq-filter
                 #'(lambda(b)
                     (let ((other-buf (my--get-buffer b)))
                       (my--base-buffer-is-p base-buf other-buf)))
                 (buffer-list))))))
      #+END_SRC
      ...
      The returned list is always non-empty: it /must/ contain the
      argument buffer.

      If the argument is a string that does not match the name of an
      existing buffer, ~nil~ is returned.
    ...
    - =my-indirect-buffer-list-with-base-buffer-of=
      ...
      #+BEGIN_SRC emacs-lisp +n -r -l ";(%s)"
        (defun my-indirect-buffer-list-with-base-buffer-of (&optional buffer-or-name)
        ...
        function to return nil if BUFFER-OR-NAME is a string.  Do not use
        this function carelessly as a predicate in a conditional or
        where/unless clause."
          (let ((buf (my--get-buffer buffer-or-name)))
      ...
      function may have returned ~nil~, given a string argument.
    ...
    #+BEGIN_SRC emacs-lisp +n -r -l ";(%s)"
      ...
      This function is intended exclusively for interactive use."
        ...
        (let ((buflist
          ...
          ;; This is kind of stringly typed, but addressing that would make
          ;; the function needlessly complex for little or no gain.
